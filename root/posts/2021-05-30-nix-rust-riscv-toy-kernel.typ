...
One of my biggest initial frustrations with embedded dev was getting a cross compiling toolchain. The "goto" cross compiler #link("https://wiki.osdev.org/GCC_Cross-Compiler")[page] is pretty intimidating for a beginner. Even now, each time I've started on an embedded project it takes me anywhere from a few hours to a week to get the new toolchain built. With `nix` this goes from an undefined amount of time to minutes.

The repo I'm using for this example is located #link("https://github.com/DieracDelta/NixKernelTutorial")[here]. Note that I am not doing anything new: several rust kernels already exist and nix has great riscv support. I'm just rehashing and hopefully explaining.

Special thanks to:

- #link("https://github.com/tock/tock")[Tock] for lots of great examples of inline assembly. They even have nix support!
- #link("https://github.com/noteed/riscv-hello-asm")[This] github repo for a bare metal example.
...
We run `import` which tells nix to execute the `nixpkgs` function in its `default.nix` and return the result. In this case, we must provide some system information in an argument to this function: specifically that our host system (denoted `localSystem`) is `x86_64` linux and our target system (denoted `crossSystem`) is riscv linux. We include the triples and ABI information:
...
`builtins.fetchurl` wgets the url prior to build time and ensures the source sha256 hash matches what we expect (e.g. the provider did not change the input source). This is a step nix requires for reproducibility. Now, from this we can just reach in and grab our modified qemu package via `pkgs.qemu`.
