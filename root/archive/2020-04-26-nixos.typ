...
I built a #link("https://github.com/DieracDelta/filehost_rust")[filehost] using Rust and Rocket.rs. This was really easy and I had no problems installing packages, building, or developing with RLS on emacs.

It's also worth mentioning that even the cross compiler stuff works well. For example, a kernel written in rust, #link("https://github.com/tock/tock/blob/master/shell.nix")[Tock], showcases how easy it is to grab a 32 bit cross compiling toolchain for RISC-V both for gcc and rustc/llvm.
...
Getting to this workable state was much easier on NixOS than doing the equivalent exercise on a fairly versatile distribution like arch or gentoo. There are already some good blog posts about this, but the gist of it for me lived in a single #link("https://github.com/DieracDelta/nix_home_manager_configs/blob/master/dotfiles/gpu_passthrough.nix")[file].
