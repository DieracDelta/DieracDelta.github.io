...
EDIT: my messy dots are #link("https://gitea.justinrestivo.me/jrestivo/neovim_dots/src/branch/master/init.vim")[HERE]
...
Now, we can make shortcuts to specific tasks by using the spacebar in a key sequence. See my #link("https://gitea.justinrestivo.me/jrestivo/neovim_dots/src/branch/master/init.vim")[config files] for some examples. Basically, we can make life a bit easier. We can split windows via these key sequences. For example, to do a vertical split, do space+w+v. This means split a window vertically. E.g. the shortcuts are mnemonic. I've included a bunch of them:
```
...
The next thing to do is get a plugin manager. Vim has plugins--bits of vimscript that provide a *lot* of additional functionality. There are a lot of plugin managers out there. Isaac recommends #link("https://github.com/junegunn/vim-plug")[vim-plug], and I've had only good experiences with it. It's incredibly simple to set up and use, but I haven't tried others because this one works so well. That being said, vim8 includes its own built-in plugin manager so if you're using vim, you may want to try that out. To install plug, follow the instructions in the README, which is just a curl command in most cases. To use, introduce a section into your config that looks like (again the README's example is great and I'm just re-explaining...):
```
...
I'll use C as the example here. First, choose a language server. I like using clangd, and that's what I'll use in this example. Clangd is part of the llvm project and most modern distros have a package that includes it. Note that there are other options such as ccls. Once you've successfully installed (or compiled) clangd, tell coc.nvim about it by including as an #link("https://github.com/neoclide/coc.nvim/wiki/Language-servers")[entry] in your coc config file (accessible via `:CocConfig`):
...
- Java: use the #link("https://github.com/eclipse/eclipse.jdt.ls")[eclipse language server]! It's great! Eclipse level autocomplete/intellisense but in vim! There are a few args you have to pass in (see my config file), and obviously you the .jar to be on your path somewhere.
...
Now, there have come times where just straight coc.nvim is good but too much effort to configure. An easier alternative is just tagging for code nav. I also find that this is good as backup in case coc.nvim doesn't have the relevant language sever. Install #link("https://github.com/universal-ctags/ctags.git")[exhuberant-ctags] and ludovicchabant/vim-gutentags. Now, you can use cntrl+\] to jump to definition.
